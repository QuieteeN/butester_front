...
import { useShareTestLinkMutation, useTestQuery } from "../../../services/testsApi";
...
const LastTestPage: React.FC = () => {
    ...
    const AnswerBox: React.FC = () => {
        const currentTest = useTestQuery();
        ...
        const createUrl = async () => {
            const data = {
                id: currentTest.data.id,
            }
            try {
                const res = await shareTest(data);
                console.log(res);
                await queryClient.invalidateQueries(['getTestById', currentTest.data.id]);
            ...
            <>
                <h2 className={classes.testHeader}>
                    ...
                    <div className={classes.btnBox}>
                        ...
                        {currentTest?.data?.url ? '' : <span title="Редактировать тест" onClick={() => navigate(`/change-test/${currentTest.data.id}`)} ><Icons.modifyProfit classes={classes.check + ' ' + classes.ban} /></span>}
